...
export function PlanCard({ plan, sessionId, level, isNested }: PlanCardProps) {
  ...
  const allEventIds = useMemo(() => plan.tasks.flatMap((t) => t.eventIds), [plan.tasks]);
  ...
  const fetchChildPlans = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/analysis?sessionId=${sessionId}&type=timeline&level=${level - 1}&limit=1`,
      );
      const data: { analyses?: Array<{ status: string; result?: { plans?: TimelinePlan[] } }> } =
        await res.json();
  ...
  return (
    ...
    >
      ...
      >
        <span className="flex-1 text-sm font-serif italic text-text-primary break-words">
          {plan.name}
        </span>
        <span
          className={clsx(
        ...
        {level > 0 && (
          <span className="text-[10px] font-mono text-text-muted shrink-0">{totalEvents}e</span>
        )}
          ...
          <div key={`task-${i}`} className="flex items-center gap-1.5 group/task">
            <div className="relative flex flex-col items-center gap-1">
              <div className="flex h-9 w-9 items-center justify-center rounded-sm border-2 border-ember bg-depth-0 transition-colors duration-300 group-hover/task:text-inherit">
                <TaskIcon
                  taskName={task.name}
                  className="h-3.5 w-3.5 text-text-secondary group-hover/task:text-inherit"
                />
              </div>
