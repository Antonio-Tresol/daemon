        ...
        {expanded && (
          <div className="border-t border-border/50 px-3 pb-3 space-y-2 depth-reveal">
            ...
            {(failure.evidence?.length > 0 || failure.eventId) && (
              <div>
                <p className="text-[9px] font-mono font-medium uppercase tracking-wider text-text-muted mb-1">Evidence</p>
                <div className="flex flex-wrap gap-1">
                  {(failure.evidence ?? (failure.eventId ? [failure.eventId] : [])).map((eid, i) => (
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <button
          ...
          className={clsx(
            ...
            filterImpact === null ? 'bg-ember/15 text-ember' : 'text-text-secondary hover:text-text-primary',
          )}
        >
          All ({failures.length})
        </button>
        ...
        {criticalCount > 0 && (
          <button
            ...
            className={clsx(
              ...
              filterImpact === 'critical' ? 'bg-ember/15 text-ember' : 'text-text-secondary hover:text-text-primary',
            )}
          >
        ...
        {warningCount > 0 && (
          <button
            ...
            className={clsx(
              'flex items-center gap-1.5 font-mono text-xs px-2 py-1 rounded-md transition-colors',
              filterImpact === 'warning' ? 'bg-ember/15 text-ember' : 'text-text-secondary hover:text-text-primary',
            )}
          >
            <span className="text-ember">{'\u25C6'}</span>
        ...
        {infoCount > 0 && (
          <button
            ...
            className={clsx(
              'flex items-center gap-1.5 font-mono text-xs px-2 py-1 rounded-md transition-colors',
              filterImpact === 'info' ? 'bg-ember/15 text-ember' : 'text-text-secondary hover:text-text-primary',
            )}
          >
            <span className="text-ember">{'\u25CB'}</span>
            {infoCount} info
